...
linkage.blocks.prominence <- function(data.obj, p.or.q = 0.05, collapse.linked.markers = TRUE, threshold.power = 2, plot.results = TRUE, verbose = FALSE){
	...
	if(!collapse.linked.markers){		
		...
		if(plot.results){
			pdf.name <- paste("Link.Block.Matrices.p.", p.or.q, ".soft.thresh.", threshold.power, ".pdf", sep = "")		
		...
		for(ch in 1:length(u_chr)){
			...
			if(u_chr[ch] == 0){
				...
				if(length(chr.markers) > 0){	
					...
					test.interactions <- get.peaks.2d.prominence(m.mat = max.mat, p.mat = min.pval, p.or.q) 
					# image(thresh.int);quartz();image(test.interactions)
					
					test.main <- get.peaks.1d.prominence(m.mat = main.effect.mat, p.mat = main.pval, p.or.q)
					# image(rotate.mat(thresh.main)); quartz(); image(rotate.mat(test.main))
					...
					if(length(cm.changes) == 0){ #if there are no changes, put the whole chromosome into the block
						...
						pdf("test.mat.pdf", width = 20, height = 10)
						fake.mat <- combined.mat; fake.mat[which(is.na(fake.mat))] <- 5
						image(x = 1:dim(combined.mat)[2], y = 1:dim(combined.mat)[1], z = rotate.mat(fake.mat), col = my.palette(50), axes = FALSE, main = paste("Combined Interactions and Main Effects Chr", u_chr[ch]), zlim = c(zmin, zmax))
						all.ind <- which(fake.mat != 0, arr.ind = TRUE)
						text(all.ind[,2], rev(all.ind[,1]), labels = signif(as.vector(fake.mat), 2), cex = 0.4)
						dev.off()
