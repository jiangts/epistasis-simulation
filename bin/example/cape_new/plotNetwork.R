...
plotNetwork <- function(data.obj, collapsed.net = TRUE, trait = NULL, phenotype.labels = NULL, main.lwd = 4, inter.lwd = 3, label.cex = 1.5, percent.bend = 15, chr.gap = 1, label.gap = 5){
			...
			for(ch in 1:length(chr.blocks.locale)){
				...
				for(ph in 1:length(pheno)){
					...
					if(main.effects[ph] != 0){ #if there are significant effects of this block, add them to the circle
						if(main.effects[ph] < 0){trait.col = "red"}else{trait.col = rgb(0, 201/256, 87/256)}
		...
		for(i in 1:dim(just.m)[1]){
			...
			if(length(sig.locale) > 0){
				...
				for(s in 1:length(sig.locale)){
					if(just.m[i,sig.locale[s]] > 0){edge.col <- rgb(0, 201/256, 87/256)}else{edge.col = "red"}
