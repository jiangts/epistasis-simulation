...
get.peaks.2d <- function(m.mat, p.mat, p.or.q, effect.drop){
	...
	check.borders <- function(peak.mat, min.effect, border.check, step.num){
		...
		if(length(min.row.check) == dim(peak.mat)[2] && border.check[1,1] == 0){
			border.check[1,1] <- step.num
			}
		#bottom row
		max.row.check <- which(peak.mat[dim(peak.mat)[1],] < min.effect)
		if(length(max.row.check) == dim(peak.mat)[2] && border.check[1,2] == 0){
	...
	for(cl in 1:comm.num){
		...
		border.check <- matrix(0, nrow = 2, ncol = 2)
		...
		while(num.borders.to.find > 0){
			...
			if(min.col == 1){border.check[2,1] <- step.num}; if(border.check[2,1] == 0){min.col <- min.col - 1}
			if(max.col == dim(m.mat)[2]){border.check[2,2] <- step.num}; if(border.check[2,2] == 0){max.col = max.col + 1}
			...
			border.check <- check.borders(peak.mat = new.peak, min.effect, border.check, step.num = step.num)
			# border.check
			num.borders.to.find <- length(which(border.check == 0))
			step.num <- step.num + 1
			}
